...
def test_sync_sources_copies_matching_files(tmp_path: Path):
    ...
    (downloads / "WhatsApp Chat - Ana.txt").write_text("hello", encoding="utf-8")
    (downloads / "ignore.zip").write_bytes(b"ignore")
    p = ensure_directories(root)
    (p.config / "config.yaml").write_text(yaml.safe_dump({"source_sync": {"enabled": True, "google_takeout_sources": [str(downloads)], "whatsapp_export_sources": [str(downloads)], "whatsapp_media_sources": []}}), encoding="utf-8")
    db.init_db(p.db)
    ...
    assert (p.whatsapp_exports_inbox / "WhatsApp Chat - Ana.txt").exists()
