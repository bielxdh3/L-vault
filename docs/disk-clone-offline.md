...
## Real artifact static-validation workflow

The pinned non-booted artifact workflow uses the Clonezilla-listed NCHC mirror for `clonezilla-live-3.3.3-15-amd64.iso`, `CHECKSUMS.TXT`, and `CHECKSUMS.TXT.gpg`, plus a fingerprint-directed public key retrieval for the DRBL signer. The cache and staging directories are private runtime data outside the repository. Downloads are HTTPS-only, host-allowlisted, size-bounded, hashed before atomic promotion, and revalidated in `--offline` mode; failed downloads remain quarantined as partial files.
...
python -m localvault disk-clone-artifacts-verify --offline --cache <PRIVATE_ARTIFACT_CACHE> --gpg <ABSOLUTE_HOST_GPG> --gpgv <ABSOLUTE_HOST_GPGV>
...
Extraction accepts only explicitly supplied, already-installed absolute extractor paths, uses `shell=False`, ordinary new directories, bounded output/time, and a second ISO hash immediately before extraction. It records the extractor identity and digest, the compressed rootfs path and digest, a schema-versioned inventory of directories, regular files, safe in-image symlinks, hardlinks, ownership/mode metadata, and special entries, and required-tool evidence as `present_unexecuted`. On Windows, an existing WSL2 `\\wsl$` ext4 tree is scanned by a fixed read-only POSIX inventory helper so Linux path semantics are not collapsed by NTFS. It never runs a file from the extracted tree. A pass returns `offline_runtime_static_validation_passed` and is reported as `real_static_runtime_validated`; it does not prove bootability, Secure Boot, VM boot, physical boot, clone correctness, or disk safety.

The official DRBL signature proves checksum-manifest provenance only. A separately protected L-vault local attestor signs the extraction attestation; it is not Clonezilla provenance. If a protected local key or a suitable ISO/SquashFS extractor is unavailable, the workflow remains blocked and no substitute release, fake signer, mount, VM, USB, disk, reboot, or clone engine is used.
