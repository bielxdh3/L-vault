...
Extraction accepts only explicitly supplied, already-installed absolute extractor paths, uses `shell=False`, ordinary new directories, bounded output/time, and a second ISO hash immediately before extraction. It records the extractor identity and digest, the compressed rootfs path and digest, a schema-versioned inventory of directories, regular files, safe in-image symlinks, and special entries, and required-tool evidence as `present_unexecuted`. It never runs a file from the extracted tree. A pass returns `offline_runtime_static_validation_passed` and is reported as `real_static_runtime_validated`; it does not prove bootability, Secure Boot, VM boot, physical boot, clone correctness, or disk safety.
