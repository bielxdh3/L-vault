...
O handoff escolhido e um USB Clonezilla Live dedicado com selecao manual de boot. Ele evita alterar permanentemente a ordem de boot e exige uma acao humana; nenhum BCD, UEFI NVRAM, BootNext, USB, particao ou PXE foi alterado. A fase atual implementa verificacao `gpgv` explicita com fingerprint completo e keyring publico somente leitura derivados do verificador usado, manifest canonico assinado, binding criptografico entre ISO verificado e inventario completo da arvore extraida, inspeção estatica de ferramentas sem execucao, canal de retorno virtual duravel e runner virtual somente-simulacao. Como nao havia ISO oficial, arvore extraida ou stack de VM segura presente, o status honesto continua `offline_runtime_blocked`; nenhum boot de VM ocorreu.
