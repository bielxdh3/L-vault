...
CLONE_STATE_LABELS = {
    ...
    "offline_runtime_synthetic_validation_passed": "fixture sintetizado validado apenas para teste; runtime de producao bloqueado",
...
def validate_disk_clone_config(config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if result["official_publisher_trust"] != "pinned_clonezilla_drbl_only" or result["local_extraction_attestation"] != "required_separate_trust_root" or result["local_signing"] != "unavailable_not_configured" or result["real_extraction"] != "disabled" or result["static_inspection"] != "only" or result["real_execution"] != "disabled":
        raise DiskCloneConfigError("A politica de confianca do runtime offline nao e allowlisted.")
    if not isinstance(result["local_attestation_fingerprint"], str) or len(result["local_attestation_fingerprint"]) > 64:
        raise DiskCloneConfigError("A evidencia do attestor local e invalida.")
    ...
    provider = _normal(result["provider"])
    if provider not in {"fake", "auto", "offline", "clonezilla_offline", "aomei", "diskgenius"}:
        raise DiskCloneConfigError("Nenhum provedor de clone configurado.")
    return result


def validated_disk_clone_config(root: Path) -> dict[str, Any]:
    return validate_disk_clone_config(load_config(root).get("disk_clone", {}))
...
def provider_for_config(config: dict[str, Any]) -> CloneProvider:
    if not isinstance(config, dict):
        raise DiskCloneConfigError("Configuracao de provedor invalida; a execucao foi bloqueada.")
    ...
    raise DiskCloneConfigError("Provedor de clone desconhecido ou nao permitido.")
...
class CloneService:
    ...
    def status(self) -> dict[str, Any]:
        ...
        if not self.config["enabled"]:
            current_state = "blocked_configuration"
    ...
    def execute(self, *, trigger: str = "scheduled", countdown: Callable[[int], str] | None = None, simulation: bool = False, parent_run_id: str | None = None) -> dict[str, Any]:
        if not simulation and not self.config["enabled"]:
            return {"state": "blocked_configuration", "reason": "A clonagem esta desativada por configuracao."}
    ...
    def retry(self, parent_run_id: str, *, countdown: Callable[[int], str] | None = None, simulation: bool = False) -> dict[str, Any]:
        ...
        if not simulation and not self.config["enabled"]:
            return {"state": "retry_rejected", "reason": "A clonagem esta desativada por configuracao."}
        with db.connect(self.p.db) as conn:
...
def disk_clone_dashboard_data(p: VaultPaths) -> dict[str, Any]:
    try:
        ...
        if not status["enabled"]:
            status["current_state"] = "blocked_configuration"
            status["current_state_label"] = "desativado por configuracao"
            status["provider_blocker"] = "A clonagem esta desativada por configuracao; status e simulacao permanecem disponiveis."
        return status
    except DiskCloneBlocked as exc:
